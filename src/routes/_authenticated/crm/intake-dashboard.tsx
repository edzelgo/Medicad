...
const searchSchema = z.object({
  ...
  caseType: z.enum(["medicaid", "caregiver"]).optional(),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {([
          { key: undefined, label: "All Cases" },
          { key: "medicaid" as const, label: "Medicaid" },
          { key: "caregiver" as const, label: "Caregiver" },
        ]).map((t) => (
          <button
            key={t.label}
            onClick={() => setSearch({ caseType: t.key, workflow: "" })}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${
              search.caseType === t.key ? "border-primary text-primary font-semibold" : "border-transparent text-muted-foreground hover:text-foreground"
            }`}
          >
            {t.label}
          </button>
              ...
              {rows.map((c: IntakeCase) => (
                <tr key={c.id}
                  ...
                  <td className="p-2 font-medium" onClick={(e) => e.stopPropagation()}>
                    <Link to="/crm/cases/$id" params={{ id: c.case_pk }} className="hover:underline">
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          {selected && (
            <>
              ...
              <Link to="/crm/cases/$id" params={{ id: selected.case_pk }} className="text-xs text-primary hover:underline">
                Open full case →
              </Link>
